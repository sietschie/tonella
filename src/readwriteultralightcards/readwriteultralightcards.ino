...
uint8_t pageAddr = 0x04; // In this example we will write/read 16 bytes (page
                         // 6,7,8 and 9). Ultraligth mem = 16 pages. 4 bytes per
                         // page. Pages 0 to 4 are for special functions.
...
void loop() {
  ...
  status = (MFRC522::StatusCode)mfrc522.MIFARE_Ultralight_Write(pageAddr,
                                                                buffer, 16);
